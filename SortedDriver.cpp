...
{
	double finalNum = number.at(0);
	double finalDelta = 0;

	if (number.size() > 2)
	{
		double prevDelta = abs(number.at(1) - number.at(0));
		double thisSmallDelta = prevDelta;

		for (int i = 1; i < number.size() - 1; i++)
		{
			double thisDelta = abs(number.at(i + 1) - number.at(i));

			if (thisDelta < prevDelta)
			{
				thisSmallDelta = thisDelta;
			}
			else
			{
				thisSmallDelta = prevDelta;
			}

			if (thisSmallDelta > finalDelta)
			{
				finalDelta = thisSmallDelta;
				finalNum = number.at(i);
			}

			prevDelta = thisDelta;
		}

		if (abs(number.at(number.size() - 1) - number.at(number.size() - 2)) > finalDelta)
		{
			finalDelta = abs(number.at(number.size() - 1) - number.at(number.size() - 2));
			finalNum = number.at(number.size() - 1);
		}
...
unmatched(list<string> & A, list<string> & B)
{
	...
	{
		for (std::list<string>::iterator is = B.begin(); is != B.end(); is++)
		{
			if (*it == *is)
			{
				counter++;
